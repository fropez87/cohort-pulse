...
if uploaded_file is not None:
    ...
    if error:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["Customer Retention %", "Customer Count", "Revenue Retention %", "Revenue $"])
        ...
        with tab3:
            st.markdown("""
            <p class="section-subtext">Percentage of initial revenue retained each month compared to Month 0.</p>
            """, unsafe_allow_html=True)
            st.dataframe(
                style_revenue_retention_table(revenue_retention_table),
                use_container_width=True,
                height=420
            )
